...
class TransactionService
{
    ...
    public function userTransactionList()
    {
        $userId = Auth::id();
    ...
    {
        $transaction = Transaction::where('type', TRANSACTION_EVENT)->with('user')->orderBy('id','DESC');
        return datatables($transaction)
            ->addColumn('amount', function ($data) {
                return showPrice($data->amount);
            })
    ...
    {
        $transaction = Transaction::where('type', TRANSACTION_MEMBERSHIP)->with('user')->orderBy('id','DESC');
        return datatables($transaction)
            ->addColumn('amount', function ($data) {
                return showPrice($data->amount);
            })
