...
class HomeService
{
    ...
    public function getPhotoGalleries(){
        $photoGallery = PhotoGallery::where('status', STATUS_ACTIVE)->get();
        return $photoGallery;
    }

    public function getAlumni($limit){
        return User::where(['users.status'=> STATUS_ACTIVE])
    ...
    public function getMembership(){
        return Membership::where('status', STATUS_ACTIVE)->orderBy('id','DESC')->get();
    }
