...
class StoryService
{
    ...
    {
        $features = Story::where('user_id', auth()->id())->orderBy('id', 'desc')->get();
    ...
    {
        $features = Story::where('status', JOB_STATUS_PENDING)->orderBy('id', 'desc')->get();
        return datatables($features)
            ->addIndexColumn()
            ->addColumn('company_logo', function ($data) {
                return '<img src="' . getFileUrl($data->company_logo) . '" alt="icon" class="rounded avatar-xs max-h-35">';
    ...
    public function update($oldSlug, $request)
    {
        ...
        try {
            ...
            $story = Story::where('slug', $oldSlug)->firstOrFail();
    ...
    {
        return Story::where('status', STATUS_ACTIVE)->paginate($limit);
    }
}
