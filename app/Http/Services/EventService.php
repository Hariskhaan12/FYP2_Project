...
class EventService
{
    ...
    {
        $eventPending = Event::where('status', STATUS_PENDING)->orderBy('id','DESC');
    ...
    {
        $allEvent = Event::where('status', STATUS_ACTIVE)->orderBy('created_at', 'desc');
    ...
    public function store($request)
    {
        try {
            DB::beginTransaction();
            if (Event::where('slug', getSlug($request->title))->withTrashed()->count() > 0) {
    ...
    {
        DB::beginTransaction();
        if (Event::where('slug', getSlug($request->title))->where('slug', '!=', $currenSlug)->withTrashed()->count() > 0) {
            $slug = getSlug($request->title) . '-' . rand(100000, 999999);
        } else {
            $slug = getSlug($request->title);
        }
        ...
        try {
            $event = Event::where('slug', $currenSlug)->with('eventTicket')->first();
