...
class NoticeCategoryService
{
    ...
    {
        $noticeCategories = NoticeCategory::query()->orderBy('id','DESC');
    ...
    {
        return NoticeCategory::join('notice', 'notice.notice_category_id', 'notice_categories.id')->where('notice.status', STATUS_ACTIVE)->groupBy('notice_categories.id')->groupBy('notice_categories.name')->select('notice_categories.name', 'notice_categories.id')->get();
    }
