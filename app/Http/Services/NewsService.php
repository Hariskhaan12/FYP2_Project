...
class NewsService
{
    ...
    {
        $features = News::with(['category', 'author'])->orderBy('id','DESC');
    ...
    public function getAllActive($limit = NULL)
    {
        ...
        if(is_null($limit)){
            return News::where('status', STATUS_ACTIVE)->where('id', '!=', $first)->with(['category', 'author'])->paginate(6);
        }else{
            return News::where('status', STATUS_ACTIVE)->limit($limit)->with(['category', 'author'])->get();
        }
    }
}
