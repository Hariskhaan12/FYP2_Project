...
class SettingController extends Controller
{
    ...
    public function configurationSettingConfigure(Request $request)
    {
        if(env('APP_DEMO', false) == false){
            ...
            <p>This is a demo version, and configuration setup is not visible.</p>
            <p>You can access the full demo after purchase or by contacting us.</p>
            <a href="https://ticket.zainikthemes.com/contact-us" target="__blank" class="btn btn-primary">Contact Us</a>
        </div>';
        }
    }
    ...
    public function tradingBotSettings()
    {
        $data['title'] = 'Trading Bot Settings';
        $data['navFeaturesParentActiveClass'] = 'mm-active';
        $data['subTradingBotActiveClass'] = 'active';
        return view('admin.setting.features_settings.trading-bot-settings', $data);
    }
    ...
    public function apiSetting()
    {
        ...
        $data['activeCoin'] = $this->settingsService->activeCoinData();
        return view('admin.setting.api_settings.api-settings', $data);
    }

    public function bitgoApiSetting()
    {
        $data['title'] = 'Api Settings';
        $data['navAPIParentActiveClass'] = 'mm-active';
        $data['subbitgoApiActiveClass'] = 'active';
        return view('admin.setting.api_settings.bitgo-api-settings', $data);
    }

    public function ERC20BEP20TRC20A()
    {
        $data['title'] = 'Api Settings';
        $data['navAPIParentActiveClass'] = 'mm-active';
        $data['sub20ApiActiveClass'] = 'active';
        return view('admin.setting.api_settings.erc-bep-trc-api-settings', $data);
    }

    public function cryproCompareApi()
    {
        $data['title'] = 'Api Settings';
        $data['navAPIParentActiveClass'] = 'mm-active';
        $data['subcryproCompareApiActiveClass'] = 'active';
        return view('admin.setting.api_settings.crypro-compare-api-settings', $data);
    }
...
//        $data['feeData'] = $this->settingsService->getFeesData();
//        $data['title'] = 'Fees Settings';
//        $data['subNavFeesSettingActiveClass'] = 'mm-active';
//        $data['subFeesActiveClass'] = 'active';
//        return view('admin.setting.fees.index', $data);

        $limits = Setting::where('option_key', 'like', 'trade_limit_%')->get();
        $makers = [];
        $takers = [];
        $trades = [];
        $numbers = [];
        foreach ($limits as $data) {
            $numbers[] = explode('_', $data->option_key)[2];
            $makers[] = 'maker_' . explode('_', $data->option_key)[2];
            $takers[] = 'taker_' . explode('_', $data->option_key)[2];
            $trades[] = 'trade_limit_' . explode('_', $data->option_key)[2];
        }
        $allOption = array_merge($makers, $takers, $trades);
        $settings = allsetting($allOption);
        $formatData = [];
    ...
    public function feesSettingsStore(Request $request)
    {
        $fields = [];
        ...
        if (!isset($request->trade_limit_1) || $request->trade_limit_1 != 0) {
            return $this->error([], __('First limit must be 0'));
        }
        $adminSettingService = new SettingsService();
        $update = $adminSettingService->tradeSetting($request->except('_token'));

        if (isset($update) && $update['success'] == true) {
            return $this->success([], getMessage(UPDATED_SUCCESSFULLY));
        }
        return $this->error([], $update['message']);
    }

    public function dbBackupSettings()
    {
        $data['title'] = __('Backup Settings');
        $data['subNavGeneralSettingActiveClass'] = 'mm-active';
