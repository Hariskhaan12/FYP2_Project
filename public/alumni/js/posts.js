(function ($) {
    ...
    window.loadComment = function(slug){
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.status == true){
                const regex = /[' ']+#+([a-zA-Z0-9_#]+)/ig;
                var commentBoxHtmlData = comment_box_html;
                var formatedCommentBoxHtmlData = commentBoxHtmlData.replace(regex, value => '<span class="text-primary fw-bold">'+value+'</span>');
                    $(document).find('.like-comment-area-'+slug).find('.post-comment-button').html(response.data.comment_button_html);
                    $(document).find('.like-comment-area-'+slug).find('.post-comment-area').html(response.data.formatedCommentBoxHtmlData);
                }
            },
            error: function (error) {
                toastr.error(error.responseJSON.message)
    ...
    $(document).on('click', '.comment-update', function(){
        ...
        let id = $(selector).data('id');
        let body = $(selector).data('comment-body');
        let route = $('#post-comment-update').val()+'?slug='+slug
